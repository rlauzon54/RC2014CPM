1 ' VERSION 2.0
2 ' COPYRIGHT, GEORGE M. RIMAKIS, 2017.
3 ' ALL RIGHTS RESERVED.
900 GOTO 14000
1000 LO=192:GOSUB 20150:GOSUB 20030:PRINT " SQUARES";
1010 LO=232:GOSUB 20150:GOSUB 20030:PRINT "   LEFT:";
1020 LO=272:GOSUB 20150:GOSUB 20030:PRINT "        ";
1030 LO=272:GOSUB 20150:GOSUB 20030:PRINT "    "+STR$(MC%);
3000 LO=(Y*40)+X:GOSUB 20150:GOSUB 20030:PRINT CHR$(255);
3002 FOR Q=0 TO 10
3004 NEXT Q
3010 LO=(Y*40)+X:GOSUB 20150:GOSUB 20030:PRINT CHR$(P%(X,Y));
3012 FOR Q=0 TO 10
3014 NEXT Q
3020 ON INSTR(1,S$,INKEY$) GOTO 3200,3500,3520,3540,3560,3600,3600,4000,4000
3200 IF MC%=0 THEN GOTO 13000
3490 GOTO 3000
3500 IF X<31 THEN X=X+1
3510 GOTO 3000
3520 IF X>0 THEN X=X-1
3530 GOTO 3000
3540 IF Y<7 THEN Y=Y+1
3550 GOTO 3000
3560 IF Y>0 THEN Y=Y-1
3570 GOTO 3000
3600 IF P%(X,Y)=239 THEN P%(X,Y)=170:GOTO 3000
3610 IF P%(X,Y)=170 THEN P%(X,Y)=239:GOTO 3000
3620 GOTO 3000
4000 IF M=1 THEN GOTO 5000
4002 LO=129:GOSUB 20150:GOSUB 20030:PRINT "CALCULATING...";
4010 I=0:N=1
4020 SEC = -60*VAL(MID$(TIME$,4,2))+VAL(RIGHT$(TIME$,2))
4030 DUMMY= RND(SE)
4040 DUMMY= RND(1)
4060 IF N>MN% THEN GOTO 4200
4070 DX= FIX(RND(1)*32)
4080 DY= FIX(RND(1)*8)
4100 IF DX<>X-1 AND DX<>X AND DX<>X+1 THEN GOTO 4110
4105 IF DY=Y-1 OR DY=Y OR DY=Y+1 THEN GOTO 4070
4110 IF S%(DX,DY)=0 THEN S%(DX,DY)=1 ELSE GOTO 4040
4120 N= N+1:GOTO 4040
4200 M=1
4300 FOR Q=0 TO 13
4400 LO=129+Q:GOSUB 20150:GOSUB 20030:PRINT CHR$(239)
4450 NEXT Q
4500 GOTO 5000
5000 IF S%(X,Y)=1 THEN GOTO 10000
5010 IF S%(X,Y)=2 THEN GOTO 3000
5020 S%(X,Y)=2:MC%=MC%-1
5050 ST%(W%,0)=X:ST%(W%,1)=Y:W%=W%+1
5060 CX=ST%(R%,0):CY=ST%(R%,1):R%=R%+1
6010 C=0
7000 IF CY-1<0 THEN GOTO 7060
7010 IF CX-1<0 THEN GOTO 7030
7020 IF S%(CX-1,CY-1)=1 THEN C=C+1:REM Check Top Left
7030 IF S%(CX,CY-1)=1 THEN C=C+1:REM Check Top Middle
7040 IF CX+1>31 THEN GOTO 7060
7050 IF S%(CX+1,CY-1)=1 THEN C=C+1:REM Check Top Right
7060 IF CX-1<0 THEN GOTO 7080
7070 IF S%(CX-1,CY)=1 THEN C=C+1:REM Check Middle Left
7080 IF CX+1>31 THEN GOTO 7100
7090 IF S%(CX+1,CY)=1 THEN C=C+1:REM Check Middle Right
7100 IF CY+1>7 THEN GOTO 7160
7110 IF CX-1<0 THEN GOTO 7130
7120 IF S%(CX-1,CY+1)=1 THEN C=C+1:REM Check Bottom Left
7130 IF S%(CX,CY+1)=1 THEN C=C+1:REM Check Bottom Middle
7140 IF CX+1>31 THEN GOTO 7160
7150 IF S%(CX+1,CY+1)=1 THEN C=C+1:REM Check Bottom Right
7160 IF C=0 THEN LO=(CY*40)+CX:GOSUB 20150:GOSUB 20030:PRINT CHR$(32);:P%(CX,CY)=32
7170 IF C>0 THEN LO=(CY*40)+CX:GOSUB 20150:GOSUB 20030:PRINT RIGHT$(STR$(C),1);:P%(CX,CY)=ASC(RIGHT$(STR$(C),1))
7200 IF C=0 THEN GOTO 8000
7300 IF R%=W% THEN GOTO 1000
7400 IF R%<W% THEN GOTO 5060
8000 IF CY-1<0 THEN GOTO 8060
8010 IF CX-1<0 THEN GOTO 8030
8020 IF S%(CX-1,CY-1)=0 THEN ST%(W%,0)=CX-1:ST%(W%,1)=CY-1:W%=W%+1:S%(CX-1,CY-1)=2:MC%=MC%-1
8030 IF S%(CX,CY-1)=0 THEN ST%(W%,0)=CX:ST%(W%,1)=CY-1:W%=W%+1:S%(CX,CY-1)=2:MC%=MC%-1
8040 IF CX+1>31 THEN GOTO 8060
8050 IF S%(CX+1,CY-1)=0 THEN ST%(W%,0)=CX+1:ST%(W%,1)=CY-1:W%=W%+1:S%(CX+1,CY-1)=2:MC%=MC%-1
8060 IF CX-1<0 THEN GOTO 8080
8070 IF S%(CX-1,CY)=0 THEN ST%(W%,0)=CX-1:ST%(W%,1)=CY:W%=W%+1:S%(CX-1,CY)=2:MC%=MC%-1
8080 IF CX+1>31 THEN GOTO 8100
8090 IF S%(CX+1,CY)=0 THEN ST%(W%,0)=CX+1:ST%(W%,1)=CY:W%=W%+1:S%(CX+1,CY)=2:MC%=MC%-1
8100 IF CY+1>7 THEN GOTO 8160
8110 IF CX-1<0 THEN GOTO 8130
8120 IF S%(CX-1,CY+1)=0 THEN ST%(W%,0)=CX-1:ST%(W%,1)=CY+1:W%=W%+1:S%(CX-1,CY+1)=2:MC%=MC%-1
8130 IF S%(CX,CY+1)=0 THEN ST%(W%,0)=CX:ST%(W%,1)=CY+1:W%=W%+1:S%(CX,CY+1)=2:MC%=MC%-1
8140 IF CX+1>31 THEN GOTO 8160
8150 IF S%(CX+1,CY+1)=0 THEN ST%(W%,0)=CX+1:ST%(W%,1)=CY+1:W%=W%+1:S%(CX+1,CY+1)=2:MC%=MC%-1
8160 IF R%<W% GOTO 5060 ELSE GOTO 1000
10000 I=0:N=0
12000 FOR N = I TO I+31
12010 IF N<40 THEN X=N:Y=0:GOTO 12110
12020 IF N<80 THEN X=N-40:Y=1:GOTO 12110
12030 IF N<120 THEN X=N-80:Y=2:GOTO 12110
12040 IF N<160 THEN X=N-120:Y=3:GOTO 12110
12050 IF N<200 THEN X=N-160:Y=4:GOTO 12110
12060 IF N<240 THEN X=N-200:Y=5:GOTO 12110
12070 IF N<280 THEN X=N-240:Y=6:GOTO 12110
12080 IF N<320 THEN X=N-280:Y=7:GOTO 12110
12110 IF S%(X,Y)=1 THEN LO=N:GOSUB 20150:GOSUB 20030:PRINT CHR$(77);
12120 NEXT N
12130 I = I + 40
12150 IF I<=280 GOTO 12000
12210 LO=32:GOSUB 20150:GOSUB 20030:PRINT "     YOU";
12220 LO=72:GOSUB 20150:GOSUB 20030:PRINT "   LOSE!";
12230 LO=112:GOSUB 20150:GOSUB 20030:PRINT "        ";
12240 LO=192:GOSUB 20150:GOSUB 20030:PRINT "     Try";
12250 LO=232:GOSUB 20150:GOSUB 20030:PRINT "  Again?";
12260 LO=272:GOSUB 20150:GOSUB 20030:PRINT "   (Y/N)";
12262 LO=312:GOSUB 20150:GOSUB 20030:PRINT "       ";
12270 A$=INKEY$
12280 IF A$="" THEN A$="Q"
12290 IF A$="Y" THEN GOTO 14100
12295 IF A$="y" THEN GOTO 14100
12300 IF A$="N" THEN END
12350 IF A$="n" THEN END
12360 GOTO 12270
13000 LO=32:GOSUB 20150:GOSUB 20030:PRINT "     YOU";
13220 LO=72:GOSUB 20150:GOSUB 20030:PRINT "    WIN!";
13230 LO=112:GOSUB 20150:GOSUB 20030:PRINT "        ";
13240 LO=192:GOSUB 20150:GOSUB 20030:PRINT "    Play";
13250 LO=232:GOSUB 20150:GOSUB 20030:PRINT "  Again?";
13260 LO=272:GOSUB 20150:GOSUB 20030:PRINT "   (Y/N)";
13262 LO=312:GOSUB 20150:GOSUB 20030:PRINT "       ";
13270 GOTO 12270
14000 SCREEN 0,0
14020 GOSUB 20010
14030 LO=94:GOSUB 20150:GOSUB 20030:PRINT "TEXT SWEEPER"
14040 LO=133:GOSUB 20150:GOSUB 20030:PRINT "BY G.M. RIMAKIS"
14050 LO=207:GOSUB 20150:GOSUB 20030:PRINT "Press [SPACEBAR] to start."
14060 S$="Q"+CHR$(32)
14070 ON INSTR(1,S$,INKEY$) GOTO 14070,14100
14080 GOTO 14070
14100 GOSUB 20010:CLEAR:S$="Q"+CHR$(28)+CHR$(29)+CHR$(31)+CHR$(30)+CHR$(70)+CHR$(102)+CHR$(32)+CHR$(13)
14110 PRINT"Use default settings?(Y/N)"
14120 A$=INKEY$
14130 IF A$="" THEN A$="Q"
14140 IF A$="Y" THEN GOTO 14200
14150 IF A$="y" THEN GOTO 14200
14160 IF A$="N" THEN GOTO 14250
14170 IF A$="n" THEN GOTO 14250
14180 GOTO 14120
14200 MN%=36:GOTO 15030
14250 GOSUB 20010:INPUT"Enter number of mines. (10-217)";MN%
14300 IF MN%<10 GOTO 14250
14350 IF MN%>217 GOTO 14250
15030 LO=32:GOSUB 20150:GOSUB 20030:PRINT "    TEXT";
15040 LO=72:GOSUB 20150:GOSUB 20030:PRINT " SWEEPER";
15080 LET I = 0
15090 LET X1%=0:Y1%=0:R%=0:W%=0
15100 M=0:MC%=256-MN%
16000 FOR N = I TO I+31
16010 LO=N:GOSUB 20150:GOSUB 20030:PRINT CHR$(239);
16020 NEXT N
16030 I = I + 40
16050 IF I<=280 GOTO 16000
16055 LO=131:GOSUB 20150:GOSUB 20030:PRINT "LOADING...";
16060 LET X=0:Y=0
16070 DIM S%(31,7):DIM P%(31,7)
16075 DIM ST%(271,1)
16080 FOR X = 0 TO 31
16090 S%(X,Y)=0
16100 NEXT X
16110 Y = Y+1
16120 IF Y <=7 GOTO 16080
16125 X=0:Y=0
16130 FOR X = 0 TO 31
16140 P%(X,Y)=239
16150 NEXT X
16160 Y = Y+1
16170 IF Y <=7 GOTO 16130
16180 X=16:Y=3
16200 LET M=0
16300 FOR Q=0 TO 9
16400 LO=131+Q:GOSUB 20150:GOSUB 20030:PRINT CHR$(239);
16450 NEXT Q
16500 GOTO 1000

20000 REM CLEAR SCREEN
20010 PRINT CHR$(27);"[2J": RETURN
20020 REM Set cursor pos ro=row, co=col
20030 RO$=STR$(RO):RO$=RIGHT$(RO$,LEN(RO$)-1)
20040 CO$=STR$(CO):CO$=RIGHT$(CO$,LEN(CO$)-1)
20050 X$=CHR$(27)+"["+RO$+";"+CO$+"H"
20060 PRINT X$;
20070 RETURN
20080 REM Reverse screen
20090 PRINT CHR$(27)+"[7m";:RETURN
20100 REM RESET ATTRIBUTES
20110 PRINT CHR$(27)+"[0m";:RETURN
20120 REM Bell
20130 PRINT CHR$(7):RETURN
20140 REM Location(LO) to row(RO) col(CO)
20150 RO=INT(LO/40)+1:CO=LO-(RO-1)*40+1
20160 RETURN
